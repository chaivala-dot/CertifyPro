...
export class StorageClient {
  ...
  private async getCertificate(hash: string): Promise<Uint8Array> {
    ...
    const body: unknown = (result as any)?.response?.body;
    const certificate = (body as any)?.certificate;
    if (certificate instanceof Uint8Array) {
      return certificate;
    }
    if (ArrayBuffer.isView(certificate)) {
      return new Uint8Array(certificate.buffer);
    }
    throw new Error("Unable to extract certificate from agent response body");
  }
